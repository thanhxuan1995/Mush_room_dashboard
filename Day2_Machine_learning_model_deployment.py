...
def read_data(url, cols):
    df = pd.read_csv(url)
    df = df[cols]  ## get only 9 important feature from Jupytor run
    
    return df

# Function to fit the LabelEncoder: encoder labeling data
@st.cache_resource
def get_target_encoder(data):
    ...
    return le
# Function to fit the OrdinalEncoder: encoder feature data
st.cache_resource
...
def encode_data(data, _X_encoder, _y_encoder):  ## _ before X_encoder to avoid streamlit hash program
    data['class'] = _y_encoder.transform(df['class'])
    X_cols = data.columns[1:]
...
def make_prediction(model, _X_encoder, X_pred):
    features = [each[0] for each in X_pred]   ## X_pred: feature input to predict
    features = np.array(features).reshape(1,-1)
    encodered_features = _X_encoder.transform(features)
    preds = model.predict(encodered_features)
...
if __name__ == "__main__":
    ...
    if pred_btn:
        ...
        # 3. Encode the data
        encode_df = encode_data(df, oe, le)
        # 4. Train the model
        gbc = train_model(encode_df)
        ...
        # 5. Make a prediction
        preds = make_prediction(gbc, oe, x_pred)
        # 6. Format the prediction to be a nice text
        nice_pred = "the mushroom is poisonous 🤢" if preds == 1 else "The mushroom is edible (●'◡'●)"
